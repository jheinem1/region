...
abstract class Region {
    ...
    abstract enteredRegion(part: BasePart, timeout?: number, step?: () => Promise<unknown>): Promise<void>;
    ...
    abstract leftRegion(part: BasePart, timeout?: number, step?: () => Promise<unknown>): Promise<void>;
...
export class BasePartRegion extends Region {
    ...
    async enteredRegion(part: BasePart, timeout?: number, step: () => Promise<unknown> = async () => wait(0.1)) {
    ...
    async leftRegion(part: BasePart, timeout?: number, step: () => Promise<unknown> = async () => wait(0.1)) {
        return new Promise<void>(async (resolve, reject, onCancel) => {
            const start = os.clock();
            let active = true;
            onCancel(() => (active = false));
...
export class RegionUnion extends Region {
    ...
    async enteredRegion(part: BasePart, timeout?: number, step: () => Promise<unknown> = async () => wait(0.1)) {
        return Promise.race(this.regions.map((region) => region.enteredRegion(part, timeout, step)));
    }
    /** Resolves when the part leaves the region and rejects if the timeout (if specified) is reached
     * @returns A promise that resolves when a player leaves a region */
    async leftRegion(part: BasePart, timeout?: number, step: () => Promise<unknown> = async () => wait(0.1)) {
        return Promise.race(this.getRegions(part.Position).map((region) => region.leftRegion(part, timeout, step)));
    }
